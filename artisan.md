...
모든 인자들을 `배 열` 형태로 확인하고자 한다면 파라미터 없이 `argument` 메소드를 호출하면 됩니다.

    $arguments = $this->arguments();

옵션을 확인하는 방법은 인자를 확인하는 것과 마찬가지로 `option` 메소드를 호출하면 됩니다. `argument` 메소드처럼 파라미터 없이 `option` 메소드를 호출하는 경우에 모든 옵션값들을 `배 열` 형태로 확인이 가능합니다.
...
아티즌 콘솔의 `make` 명령어들은 컨트롤러, 마이그레이션, job, 테스트 같은 다양한 종류의 클래스들을 생성하는 데 사용됩니다. 이 클래스들은 당신의 입력값에 따라 값이 채워지는 "stub" 파일들을 통해 생성됩니다. 하지만, 당신은 때때로 아티즌에 의해 생성된 파일에 약간의 변화를 주고 싶을 수도 있습니다. 이를 위해 `stub:publish` 명령어를 사용해서 커스터마이징을 위한 가장 기본적인 stub를 만들 수 있습니다.

    php artisan stub:publish

생성된 stub들은 당신의 애플리케이션 디렉토리 루트에 있는 `stubs` 디렉터리에 저장될 것입니다. 이 stub들의 수정 사항은 아티즌 `make` 명령어를 실행할 때 상응하는 클래스에 반영될 것입니다.
