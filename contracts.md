...
라라벨의 [파사드](/docs/{{version}}/facades)는 서비스 컨테이너 외부에서 타입 힌트나, contracts 의 의존성 없이도 라라벨의 서비스를 시용할 수 있는 쉬운 방법을 제공합니다. 그에 비해 contracts 를 사용함으로써 클래스의 명확한 의존성을 정의할 수 있습니다. 대부분의 애플리케이션에서 파사드를 사용하는 것은 문제가 되지 않습니다. 하지만 느슨한 결합이 필요한 경우 contract 가 유용합니다. 계속 읽어 보세요!
...
이벤트 리스너가 의존성 해결될 때, 서비스 컨테이너는 클래스의 생성자에 있는 타입힌트를 읽고, 그에 적합한 값을 주입해 줍니다. 서비스 컨테이너에 무언가를 등록하는 것에 대하여 더 알고싶다면, [이 문서](/docs/{{version}}/container)를 보시기 바랍니다.
