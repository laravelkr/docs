...
소프트 삭제가 적용된 모델의 쿼리에 `orWhere` 절을 추가할 때는 `WHERE`를 통해서 모델들을 걸러내기 위해서 항상 [advance where clauses](http://laravel.com/docs/5.1/queries#advanced-where-clauses)를 사용합니다. 예로 들어: 
...
`orWhere` 절이 그룹화되지 않았다면 소프트 삭제된 레코드들을 포함하는 다음의 SQL을 생성할 것입니다: 
...
`withTrashed` 메소드와 같이 `restore` 메소드도 [관계](/docs/{{version}}/eloquent-relationships)에 쓰일 수 있습니다: 
...
스코프는 애플리케이션에서 손쉽게, 반복적으로 사용할 수 있는 공통의 범위 제한을 정의할 수 있게 해줍니다. 예로 들어 여러분은 종종 "인기가 높은" 것으로 생각되는 사용자를  조회해야 한다고 합시다. 스코프를 정의하기 위해서는 간단하게 Eloquent 메소드의 이름에 `scope` 를 접두어로 붙이면 됩니다.
