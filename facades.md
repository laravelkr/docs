...
실시간 파사드를 사용하여 어플리케이션의 모든 클래스를 파사드처럼 취급 할 수 있습니다. 어떻게 이를 사용할 수 있는지 알기 위해서, 다음의 경우를 살펴보겠습니다. 예를 들어 `Podcast` 모델이 `publish` 메소드를 가지고 있다고 가정해보겠습니다. 이때, podcast를 publish 하기 위해서는 `Publisher` 인스턴스를 주입해야 합니다:
