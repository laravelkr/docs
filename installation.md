...
> {video} 설치 영상을 확인하고 싶다면? 라라캐스트에서 프레임워크를 새롭게 배우는 사람들을 위한 [무료 라라벨 소개 영상](https://laracasts.com/series/laravel-from-scratch-2017)을 제공하고 있습니다. 라라벨을 시작하는데 참고하시기 바랍니다.
...
로컬환경에 PHP가 설치되어 있다면 PHP의 내장 개발 서버를 사용하여 애플리케이션을 구동하고자 할 수 있는데, 이경우 `serve` 아티즌 명령어를 사용할 수 있습니다. 이 명령어는 개발서버를 `http://localhost:8000`으로 시작할 것입니다:
...
라라벨을 설치후에 애플리케이션의 document / web 루트를 `public` 디렉토리로 설정해야 합니다. 이 디렉토리의 `index.php`은 애플리케이션의 모든 HTTP 요청에 대해서 프론트 컨트롤러로 작동합니다.
