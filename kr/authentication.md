...
지정되는 guard 는 `auth.php` 설정 파일의 `guard` 배열안에 있는 키 중에 하나와 일치해야 합니다. 
...
라라벨은 사용자 패스워드 재설정에 필요한 로직을 포함하는 `Auth\PasswordController`를 가지고 있습니다. `make:auth` 아티즌 명령어를 사용하여 암호 재설정을 수행하는데 필요한 모든 라우트들을 생성할 수 있습니다:
...
사용자의 패스워드를 재설정하기 위해 라우트와 뷰를 정의하였다면, 이제 브라우저에서 `/password/reset`로 접근하여 라우트에 접근할 수 있습니다. 프레임워크에 포함된 `PasswordController`는 이미 패스워드 재설정 이메일을 보내고, 데이터베이스에 패스워드를 업데이트하는 로직을 가지고 있습니다.
...
이 예제에서 볼 수 있듯이, `extend` 메소드에 전달된 콜백은 `Illuminate\Contracts\Auth\Guard` 구현체를 반환해야 합니다. 이 인터페이스는 사용자 정의 guard를 구현하는데 필요한 몇가지 메소드를 가지고 있습니다. 

Once your custom guard has been defined, you may use the guard in your `guards` configuration:

사용자 정의 guard 를 정의하고나면, `guards` 설정에서 정의한 guard 를 사용할 수 있습니다: 
...
그리고, `guards` 설정 에서 이 프로바이더를 사용할 수 있습니다. 
