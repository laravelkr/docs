...
- [Creating Facades 파사드 생성하기](#creating-facades)
- [Mocking Facades 파사드 Mocking](#mocking-facades)
- [Facade Class Reference 파사드 클래스 목록](#facade-class-reference)
...
> **주의** 파사드에 대해서 알아보기 전에 라라벨의 [서비스 컨테이너](/docs/{{version}}/container)에 익숙질 것을 적극 권장합니다. 
...
바인딩을 추가하는 좋은 방법은 `PaymentServiceProvider`라는 [서비스 프로바이더](/docs/{{version}}/container#service-providers)를 생성하고 `register` 메소드에서 바인딩을 추가하는 것입니다.  그 다음에 `config/app.php` 설정 파일에서 라라벨에서 로딩하는 서비스 프로바이더 항목을 설정합니다. 
