...
`config/horizon.php` 설정 파일에 "긴 대기시간"으로 간주하는 시간 설정 할 수 있습니다. 파일 내의 `waits` 설정 옵션에서 각 연결/queue 조합에 대한 긴 대기 기준 값을 제어 할 수 있습니다 :
...
Horizon에는 작업 및 queue 대기 시간과 처리량에 대한 정보를 제공하는 메트릭 dashboard가 포함되어 있습니다, 이 dashboard를 채우려면 애플리케이션의 [scheduler](/docs/{{version}}/scheduling)를 통해 5분마다 실행되도록 Horizon의 `snapshot` Artisan 커맨드를 구성해야 합니다:
