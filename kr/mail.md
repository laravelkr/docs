...
이메일을 발송하는 대신, `log` 메일 드라이버는 모든 이메일 메세지를 확인하기 위해서 로그파일에 기록합니다. 보다 구동 환경별 어플리케이션을 설정하는 보다 자세한 정보는 [설정 매뉴얼](/docs/{{version}}/installation#environment-configuration)을 확인하십시오.
