...
[이메일 발송](/docs/{{version}}/mail)기능 이외에도 Laravel은 이메일, SMS ([Vonage](https://www.vonage.com/communications-apis/) 이전에 Nexmo로 알려진 서비스 제공회사) 또는 [Slack](https:slack.com) 를 포함한 다양한 전달 방법들을 통해 알림 전송기능을 제공합니다. 또한 수십 개의 다른 채널로 알림을 보낼 수 있도록 다양한 [커뮤니티 제작 알람 채널](https://laravel-notification-channels.com/about/#suggesting-a-new-channel) 만들어져 있습니다. 물론, 알림은 데이터베이스에 저장되어 웹 인터페이스에 표시될 수도 있습니다.
...
일반적으로 알람은 어플리케이션에서 발생하는 무언가를 사용자에게 공지하는 정보성 메시지 짧은 메시지 입니다. 예를들어, 당신이 결재 어플리케이션을 작성하고 있다면, 당신은 "결제 영수증" 알람을 이메일이나 SMS 채널을 통해 보내야할 것입니다.
...
라라벨에서 각각의 알람은 하나의 단독 클래스로 표현되고 `app/Notifications` 디렉토리에 저장됩니다. 당신의 어플리케이션에서 이 디렉토리를 보지 못했다고 걱정하지 마세요 - 이것은 당신이 아티잔 커맨드 `make:notification` 을 실행할 때 만들어 집니다.
...
이 커멘드는 깨끗한 알람 클래스를 당신의 `app/Notifications` 디렉토리에 생성합니다. 각 알람 클래스는 `via` 메소드와 알람을 해당되는 특정 채널에 맞게 조정된 메시지로 변환하는 `toMail` or `toDatabase` 같은 여러개의 메시지 빌드용 메소드를 포함합니다.
...
알람은 두가지 방법으로 보낼 수 있습니다. `Notifiable` 트레이스의 `notify` 메소드를 이용하거나 `Notification` [파사드](/docs/{{version}}/facades) 를 이용합니다. `Notifiable` 트레이스는 어플리케이션에 `App\Models\User` 모델을 기본적으로 포함하고 있습니다.
...
다른 대안으로 당신은 `Notification` [파사드](/docs/{{version}}/facades)를 이용해 알람을 보낼 수 있습니다. 이 방식은 유저 집합과 같은 다수의 단체에 알람을 보낼때 필요합니다. 파사드를 이용해 알람을 보내기 위해서는 모든 알람 수신자와 Notification 인스턴스를 `send` 메소드에 넘겨줘야 합니다.
...
모든 알림 클래스에는 알림을 전달할 채널을 결정하는 'via' 메소드가 있습니다. 알림은 `mail`, `database`, `broadcast`, `nexmo` 및 `slack` 채널에 보낼 수 있습니다.
...
> {tip} 텔레그렘이나 푸셔와 같은 다른 전달 채널을 사용하고 싶다면 커뮤니티 주도의 커뮤니티를 확인하세요. [Laravel Notification Channels website](http://laravel-notification-channels.com).

The `via` method receives a `$notifiable` instance, which will be an instance of the class to which the notification is being sent. You may use `$notifiable` to determine which channels the notification should be delivered on:

`via` 메소드는 알림이 전송되는 클래스의 인스턴스인 `$notifiable` 인스턴스를 수신합니다. 알림이 전달되어야 하는 채널을 결정하기 위해 `notifiable`을 사용할 수 있습니다:
...
> {note} 알림을 대기열에 추가하기 전에 대기열을 설정하고 [작업 시작](/docs/{{version}}/대기열)을 설정해야 합니다.
...
`ShouldQueue` 인터페이스가 알림에 추가되면 평소처럼 알림을 보낼 수 있습니다. Laravel은 클래스의 'ShouldQueue' 인터페이스를 감지하고 자동으로 알림 전달을 대기열에 넣습니다.
...
알림 전달을 지연하려면 `delay` 메소드를 알림 인스턴스화에 연결할 수 있습니다.
...
알림에서 지원하는 각 알림 채널에 사용해야 하는 특정 대기열을 지정하려면 알림에 `viaQueues` 메소드를 정의할 수 있습니다. 이 메소드는 채널 이름 => 큐 이름 쌍의 배열을 반환해야 합니다.
...
> {tip} 우리는 `toMail` 메소드에서 `this->invoice->id`를 사용하고 있습니다. 알림 클래스 생성자에 알림 메시지를 생성하는 데 필요한 모든 데이터를 전달할 수 있습니다.
...
알림 클래스에서 텍스트의 "lines"을 정의하는 대신 `view` 메소드를 사용하여 알림 이메일을 렌더링하는 데 사용할 커스텀 템플릿( 라라벨 블레이드 탬플릿 )을 지정할 수 있습니다.
...
기본적으로 이메일의 발신인 주소는 `config/mail.php` 설정 파일에 정의되어 있습니다. 그러나 `from` 메소드를 사용하여 특정 알림에 대한 보낸사람 주소를 지정할 수 있습니다.
...
알림 패키지의 리소스를 게시하여 메일 알림에 사용되는 HTML 및 일반 텍스트 템플릿을 수정할 수 있습니다. 이 명령을 실행한 후 메일 알림 템플릿은 `resources/views/vendor/notifications` 디렉토리에 있습니다.
...
Markdown 메일 알림을 사용하면 미리 작성된 메일 알림 템플릿을 활용할 수 있으며 더 길고 커스터마이징된 메시지를 자유롭게 작성할 수 있습니다. 메시지가 Markdown으로 작성되기 때문에 Laravel은 메시지에 대해 아름답고 반응형인 HTML 템플릿을 렌더링할 수 있으며 동시에 일반 텍스트도 자동으로 생성할 수 있습니다.
...
해당 Markdown 템플릿으로 알림을 생성하려면 `make:notification` Artisan 명령의 `--markdown` 옵션을 사용할 수 있습니다.
...
Markdown 메일 알림은 Blade 컴포넌트와 Markdown 구문의 조합을 사용하여 Laravel의 사전 제작된 알림 컴포넌트를 활용하면서 알림을 쉽게 설정할 수 있도록 합니다.
...
패널 컴포넌트는 알림의 나머지 부분과 배경색이 약간 다른 패널에서 주어진 텍스트 블록을 렌더링합니다. 이렇게 하면 주어진 텍스트 블록에 주의를 끌 수 있습니다.
...
테이블 컴포넌트를 사용하면 Markdown 테이블을 HTML 테이블로 변환할 수 있습니다. 컴포넌트는 Markdown 테이블을 콘텐츠로 받아들입니다. 기본 Markdown 테이블 정렬 구문을 사용하여 테이블 열 정렬이 지원됩니다.
...
모든 Markdown 알림 컴포넌트를 사용자 지정을 위해 자체 애플리케이션으로 내보낼 수 있습니다. 컴포넌트를 내보내려면 `vendor:publish` Artisan 명령을 사용하여 `laravel-mail` 에셋 태그를 게시합니다.

    php artisan vendor:publish --tag=laravel-mail

This command will publish the Markdown mail components to the `resources/views/vendor/mail` directory. The `mail` directory will contain an `html` and a `text` directory, each containing their respective representations of every available component. You are free to customize these components however you like.

이 커멘드는 Markdown 메일 컴포넌트를 `resources/views/vendor/mail` 디렉토리에 게시합니다. `mail` 디렉토리에는 `html`과 `text` 디렉토리가 포함되며, 각각은 사용 가능한 모든 설정요소에 대한 각각의 표현을 포함합니다. 이러한 컴포넌트를 원하는 대로 자유롭게 사용자 지정할 수 있습니다.
...
컴포넌트를 내보낸 후 `resources/views/vendor/mail/html/themes` 디렉토리에 `default.css` 파일이 포함됩니다. 이 파일에서 CSS를 커스터마이징할 수 있으며 스타일은 Markdown 알림의 HTML 표현 내에 자동으로 인라인됩니다.
...
Laravel의 Markdown 컴포넌트에 대한 완전히 새로운 테마를 빌드하려면 `html/themes` 디렉토리에 CSS 파일을 배치할 수 있습니다. CSS 파일의 이름을 지정하고 저장한 후 `mail` 설정 파일의 `theme` 옵션을 새 테마의 이름과 일치하도록 업데이트합니다.
...
개별 알림의 테마를 사용자 지정하려면 알림의 메일 메시지를 작성하는 동안 `theme` 메소드를 호출할 수 있습니다. `theme` 메소드는 알림을 보낼 때 사용해야 하는 테마의 이름을 받습니다.
...
알림이 데이터베이스 테이블에 저장되는 것을 지원하는 경우 알림 클래스에 `toDatabase` 또는 `toArray` 메소드를 정의해야 합니다. 이 메소드는 `notifiable` 엔티티를 수신하고 일반 PHP 배열을 리턴해야 합니다. 반환된 배열은 JSON으로 인코딩되어 `notifications` 테이블의 `data` 열에 저장됩니다. `toArray` 메소드의 예를 살펴보겠습니다.
...
일반적으로 사용자가 알림을 볼 때 알림을 "읽음"으로 표시하려고 합니다. `Illuminate\Notifications\Notifiable` 특성은 알림의 데이터베이스 레코드에서 `read_at` 열을 업데이트하는 `markAsRead` 메소드를 제공합니다.
...
그러나 각 알림을 반복하는 대신 알림 컬렉션에서 직접 `markAsRead` 메소드를 사용할 수 있습니다.

    $user->unreadNotifications->markAsRead();

You may also use a mass-update query to mark all of the notifications as read without retrieving them from the database:

또한 대량 업데이트 쿼리를 사용하여 데이터베이스에서 검색하지 않고 모든 알림을 읽은 것으로 표시할 수 있습니다.
...
알림을 `delete` 하여 테이블에서 완전히 제거할 수 있습니다.
...
모든 브로드캐스트 알림은 브로드캐스트를 위해 대기열에 있습니다. 브로드캐스트 작업을 대기열에 넣는 데 사용되는 대기열 연결 또는 대기열 이름을 설정하려면 `BroadcastMessage`의 `onConnection` 및 `onQueue` 메소드를 사용할 수 있습니다.
...
SMS 메시지에 유니코드 문자가 포함될 경우 `NexmoMessage` 인스턴스를 생성할 때 `unicode` 메소드를 호출해야 합니다.
...
`config/services.php` 파일에 지정된 전화번호와 다른 전화번호로 알림을 보내려면 `NexmoMessage` 인스턴스에서 `from` 메소드를 호출하면 됩니다.
...
`from` 및 `to` 메소드를 사용하여 보낸 사람과 받는 사람을 사용자 지정할 수 있습니다. `from` 메소드는 사용자 이름과 이모티콘 식별자를 허용하는 반면 `to` 메소드는 채널 또는 사용자 이름을 허용합니다.
...
Slack 메시지에 "attachments"을 추가할 수도 있습니다. 첨부 파일은 단순한 문자 메시지보다 더 풍부한 서식 옵션을 제공합니다. 이 예에서는 예외에 대한 자세한 내용을 볼 수 있는 링크를 포함하여 애플리케이션에서 발생한 예외에 대한 오류 알림을 보냅니다.
...
첨부 파일을 사용하면 사용자에게 표시해야 하는 데이터 배열을 지정할 수도 있습니다. 주어진 데이터는 쉽게 읽을 수 있도록 표 스타일 형식으로 표시됩니다.
...
일부 첨부 파일 필드에 Markdown이 포함되어 있는 경우 'markdown' 메소드를 사용하여 Slack에 지정된 첨부 파일 필드를 구문 분석하고 Markdown 형식의 텍스트로 표시하도록 지시할 수 있습니다. 이 방법에서 허용되는 값은 'pretext', 'text' 및 또는 'fields'입니다. Slack 첨부 파일 형식에 대한 자세한 내용은 [Slack API 문서](https//:api.slack.com/docs/message-formatting#message_formatting)를 확인하세요.
...
> {tip} `EventServiceProvider`에 리스너를 등록한 후 `event:generate` Artisan 명령을 사용하여 리스너 클래스를 빠르게 생성하세요.
...
이벤트 리스너 내에서 이벤트의 `notifiable`, `notification`, `channel` 및 `response` 속성에 액세스하여 알림 수신자 또는 알림 자체에 대해 자세히 알아볼 수 있습니다.
...
Laravel은 소수의 알림 채널과 함께 제공되지만 다른 채널을 통해 알림을 전달하기 위해 고유한 드라이버를 작성하고 싶을 수도 있습니다. Laravel은 이를 간단하게 만듭니다. 시작하려면 `send` 메소드를 포함하는 클래스를 정의하십시오. 이 메소드는 `notifiable`과 `notification`의 두 가지 인수를 받아야 합니다.
