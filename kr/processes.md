...
라라벨은 [Symfony Process component](https://symfony.com/doc/current/components/process.html)를 기반으로 표현력이 풍부하고 최소한의 API를 제공합니다. 이를 통해 라라벨 애플리케이션에서 외부 프로세스를 편리하게 호출할 수 있습니다. 라라벨의 프로세스 기능은 가장 일반적인 사용 사례에 초점을 맞추고 있으며 개발자 경험도 훌륭합니다.
...
프로세스를 호출하려면 `Process` 파사드가 제공하는 `run` 및 `start` 메서드를 사용할 수 있습니다. `run` 메서드는 프로세스를 호출하고 프로세스가 실행을 완료할 때까지 기다립니다. `start` 메서드는 비동기 프로세스 실행에 사용됩니다. 이 문서에서 두 가지 접근 방식을 모두 살펴보겠습니다. 먼저 기본적인 동기 프로세스를 호출하고 결과를 검사하는 방법을 살펴보겠습니다.
...
프로세스 결과가 있고 종료 코드가 0보다 크면(즉, 실패를 나타내는 경우) `Illuminate\Process\Exceptions\ProcessFailedException` 인스턴스를 throw하려면 `throw` 및 `throwIf` 메서드를 사용할 수 있습니다. 프로세스가 실패하지 않으면 프로세스 결과 인스턴스가 반환됩니다.
...
물론, 프로세스를 호출하기 전에 프로세스의 동작을 사용자 정의해야 할 수도 있습니다. 다행히도 라라벨은 작업 디렉토리, 타임아웃, 환경 변수와 같은 다양한 프로세스 기능을 조정할 수 있도록 해줍니다.
...
기본적으로 프로세스는 60초 이상 실행한 후 `Illuminate\Process\Exceptions\ProcessTimedOutException` 인스턴스를 throw합니다. 그러나 `timeout` 메서드를 통해 이 동작을 사용자 정의할 수 있습니다.
...
환경 변수는 `env` 메서드를 통해 프로세스에 제공될 수 있습니다. 호출된 프로세스는 시스템에서 정의한 모든 환경 변수를 상속합니다:
...
`tty` 메서드를 사용하여 프로세스에 TTY 모드를 활성화할 수 있습니다. TTY 모드는 프로세스의 입력과 출력을 프로그램의 입력과 출력에 연결하여 프로세스가 프로세스로 Vim이나 Nano와 같은 편집기를 열 수 있도록 합니다:
...
이전에 설명한 바와 같이, 프로세스 출력은 프로세스 결과의 `output`(stdout) 및 `errorOutput`(stderr) 메서드를 사용하여 액세스할 수 있습니다:
...
그러나 출력은 `run` 메서드의 두 번째 인수로 클로저를 전달하여 실시간으로 수집할 수도 있습니다. 클로저는 두 개의 인수를 받습니다: 출력의 "유형"(`stdout` 또는 `stderr`)과 출력 문자열 자체입니다:
...
라라벨은 `seeInOutput` 및 `seeInErrorOutput` 메서드도 제공합니다. 이 메서드는 주어진 문자열이 프로세스의 출력에 포함되어 있는지 여부를 확인하는 편리한 방법을 제공합니다:
...
프로세스가 관심 없는 많은 양의 출력을 작성하는 경우, 출력 검색을 완전히 비활성화하여 메모리를 절약할 수 있습니다. 이를 위해 프로세스를 빌드하는 동안 `quietly` 메서드를 호출하면 됩니다:
...
때로는 한 프로세스의 출력을 다른 프로세스의 입력으로 만들고 싶을 수 있습니다. 이를 종종 프로세스의 출력을 다른 프로세스로 "파이프링"이라고 합니다. `Process` 파사드가 제공하는 `pipe` 메서드를 사용하면 이를 쉽게 수행할 수 있습니다. `pipe` 메서드는 파이프된 프로세스를 동기적으로 실행하고 파이프라인의 마지막 프로세스에 대한 프로세스 결과를 반환합니다:
...
파이프라인을 구성하는 개별 프로세스를 사용자 정의할 필요가 없으면 `pipe` 메서드에 명령 문자열의 배열을 전달하기만 하면 됩니다:
...
`pipe` 메서드의 두 번째 인수로 클로저를 전달하여 프로세스 출력을 실시간으로 수집할 수도 있습니다. 클로저는 두 개의 인수를 받습니다: 출력의 "유형"(`stdout` 또는 `stderr`)과 출력 문자열 자체입니다:
...
라라벨은 `as` 메서드를 통해 파이프라인 내의 각 프로세스에 문자열 키를 할당할 수도 있습니다. 이 키는 `pipe` 메서드에 제공된 출력 클로저에도 전달되므로 출력이 어느 프로세스에 속하는지 확인할 수 있습니다:
...
`run` 메서드는 프로세스를 동기적으로 호출하지만 `start` 메서드는 프로세스를 비동기적으로 호출하는 데 사용할 수 있습니다. 이를 통해 프로세스가 백그라운드에서 실행되는 동안 애플리케이션이 다른 작업을 계속 수행할 수 있습니다. 프로세스가 호출되면 `running` 메서드를 사용하여 프로세스가 여전히 실행 중인지 확인할 수 있습니다:
...
`id` 메서드를 사용하여 실행 중인 프로세스의 운영 체제 할당 프로세스 ID를 조회할 수 있습니다:
...
비동기 프로세스가 실행되는 동안 `output` 및 `errorOutput` 메서드를 사용하여 전체 현재 출력에 액세스할 수 있지만, `latestOutput` 및 `latestErrorOutput`을 사용하여 마지막으로 출력을 검색한 이후에 발생한 프로세스의 출력에 액세스할 수 있습니다:
...
`run` 메서드와 마찬가지로, `start` 메서드의 두 번째 인수로 클로저를 전달하여 비동기 프로세스에서 실시간으로 출력을 수집할 수도 있습니다. 클로저는 두 개의 인수를 받습니다: 출력의 "유형"(`stdout` 또는 `stderr`)과 출력 문자열 자체입니다:
...
라라벨은 동시에 여러 작업을 쉽게 실행할 수 있도록 동시 비동기 프로세스 풀을 관리하는 것도 쉽게 만듭니다. 시작하려면 `Illuminate\Process\Pool` 인스턴스를 받는 클로저를 인수로 받는 `pool` 메서드를 호출하면 됩니다.
...
보다시피 `wait` 메서드를 통해 풀 프로세스가 모두 실행을 완료하고 결과를 해결할 수 있습니다. `wait` 메서드는 키를 통해 풀의 각 프로세스의 프로세스 결과 인스턴스에 액세스할 수 있도록 하는 배열 액세스 가능한 객체를 반환합니다:
...
또는, 편의상 `concurrently` 메서드를 사용하여 비동기 프로세스 풀을 시작하고 즉시 결과를 기다릴 수 있습니다. 이는 PHP의 배열 해체 기능과 결합할 때 특히 표현력 있는 구문을 제공할 수 있습니다:
...
숫자 키를 통해 프로세스 풀 결과에 액세스하는 것은 표현력이 떨어집니다. 따라서 라라벨은 `as` 메서드를 통해 풀 내의 각 프로세스에 문자열 키를 할당할 수 있습니다. 이 키는 `start` 메서드에 제공된 클로저에도 전달되므로 출력이 어느 프로세스에 속하는지 확인할 수 있습니다:
...
프로세스 풀의 `running` 메서드는 풀 내에서 호출된 모든 프로세스의 컬렉션을 제공하므로 백그라운드 풀 프로세스 ID에 쉽게 액세스할 수 있습니다:
...
그리고 편의상, 프로세스 풀에서 `signal` 메서드를 호출하여 풀 내의 모든 프로세스에 시그널을 보낼 수 있습니다:
...
많은 라라벨 서비스는 테스트를 쉽고 표현력 있게 작성할 수 있도록 도와주는 기능을 제공합니다. Laravel의 프로세스 서비스도 예외는 아닙니다. `Process` 파사드의 `fake` 메서드를 사용하면 프로세스가 호출될 때 라라벨이 스텁 / 더미 결과를 반환하도록 지시할 수 있습니다.
...
라라벨의 프로세스 가짜 기능을 살펴보기 위해 프로세스를 호출하는 라우트를 상상해 보겠습니다:
...
이 라우트를 테스트할 때 `Process` 파사드의 `fake` 메서드를 인수 없이 호출하여 호출된 모든 프로세스에 대해 가짜 성공 프로세스 결과를 반환하도록 라라벨에 지시할 수 있습니다. 또한, 주어진 프로세스가 "실행"되었는지 [assert](#available-assertions)할 수도 있습니다:
...
이전에 설명한 바와 같이 `Process` 파사드의 `fake` 메서드를 호출하면 라라벨이 항상 출력이 없는 성공적인 프로세스 결과를 반환하도록 지시합니다. 그러나 `Process` 파사드의 `result` 메서드를 사용하여 가짜 프로세스의 출력 및 종료 코드를 쉽게 지정할 수 있습니다:
...
이전 예제에서 알 수 있듯이 `Process` 파사드는 `fake` 메서드에 배열을 전달하여 프로세스별로 다른 가짜 결과를 지정할 수 있습니다.
...
배열의 키는 가짜를 지정하려는 명령 패턴과 연결된 결과를 나타내야 합니다. `*` 문자는 와일드카드 문자로 사용할 수 있습니다. 가짜가 아닌 모든 프로세스 명령은 실제로 호출됩니다. `Process` 파사드의 `result` 메서드를 사용하여 이러한 명령에 대한 스텁 / 가짜 결과를 구성할 수 있습니다:
...
가짜 프로세스의 종료 코드 또는 오류 출력을 사용자 정의할 필요가 없으면 가짜 프로세스 결과를 간단한 문자열로 지정하는 것이 더 편리할 수 있습니다:
...
테스트하는 코드가 동일한 명령으로 여러 프로세스를 호출하는 경우 각 프로세스 호출에 다른 가짜 프로세스 결과를 할당할 수 있습니다. `Process` 파사드의 `sequence` 메서드를 사용하여 이를 수행할 수 있습니다:
...
지금까지 `run` 메서드를 사용하여 동기적으로 호출되는 프로세스를 위조하는 것에 대해 주로 논의했습니다. 그러나 `start`를 통해 호출된 비동기 프로세스와 상호 작용하는 코드를 테스트하려는 경우 가짜 프로세스를 설명하는 더 정교한 접근 방식이 필요할 수 있습니다.

For example, let's imagine the following route which interacts with an asynchronous process:

예를 들어, 다음과 같은 비동기 프로세스와 상호 작용하는 라우트를 상상해 보겠습니다:
...
이 프로세스를 올바르게 위조하려면 `running` 메서드가 `true`를 반환하는 횟수를 설명할 수 있어야 합니다. 또한, 순차적으로 반환되어야 하는 여러 줄의 출력을 지정할 수도 있습니다. 이를 위해 `Process` 파사드의 `describe` 메서드를 사용할 수 있습니다:
...
위의 예제를 살펴보겠습니다. `output` 및 `errorOutput` 메서드를 사용하여 순차적으로 반환되는 여러 줄의 출력을 지정할 수 있습니다. `exitCode` 메서드를 사용하여 가짜 프로세스의 최종 종료 코드를 지정할 수 있습니다. 마지막으로 `iterations` 메서드를 사용하여 `running` 메서드가 `true`를 반환하는 횟수를 지정할 수 있습니다.
...
이전에 [설명한 바](#가짜-프로세스)와 같이 라라벨은 기능 테스트에 대한 여러 프로세스 어설션을 제공합니다. 아래에서 각 어설션에 대해 설명하겠습니다.
...
개별 테스트 또는 전체 테스트 스위트에서 호출된 모든 프로세스가 위조되었는지 확인하려면 `preventStrayProcesses` 메서드를 호출할 수 있습니다. 이 메서드를 호출한 후 해당 가짜 결과가 없는 프로세스는 실제 프로세스를 시작하는 대신 예외를 throw합니다:
