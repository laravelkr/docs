...
큐의 환경 설정 파일은 `config/queue.php`에 위치하고 있습니다. 이 파일 안에서 프레임워크에 포함 되어있는 각각의 큐 드라이버인 database, [Beanstalkd](http://kr.github.com/beanstalkd), [IronMQ](http://iron.io), [Amazon SQS](http://aws.amazon.com/sqs), [Redis](http://redis.io), null, 그리고 synchronous (로컬 사용을 위한) 의 연결 설정을 확인할 수 있습니다.
...
`database` 큐 드라이버를 사용하기 위해서는 작업들을 담을 데이터베이스 테이블이 필요합니다. 이 테이블을 생성하기위한 마이그레이션을 생성하려면 `queue:table` 아티즌 명령을 실행해야 합니다.
...
아래의 dependency들은 언급했던 큐 드라이버들을 사용하는데 필요합니다.
...
여러분의 어플리케이션에서 큐로 할 수 있는 모든 작업들은 `App\Commands` 폴더 안에 저장됩니다. 새로운 대기 명령은 아티즌 CLI를 사용하여 만들 수 있습니다:
...
> **노트:** 이 예제에서는 `Queue` facade를 직접적으로 사용하고 있습니다. 그러나 일반적으로 [Command Bus](/docs/5.0/bus)를 통해 큐 명령을 보냅니다. 이전부터 여러분의 어플리케이션에 queued and synchronous 명령들을 보내왔기 이 페이지에서는 `Queue` facade를 계속해서 사용할것입니다. 그러나 커멘드 버스(Command Bus)를 사용하는것도 익숙해져야 합니다.
...
기본적으로 `make:command` 아티즌 커멘드는 `handle` 메소드가 커멘드에 직접 추가되는  “self-handling” 명령을 생성합니다. 이 메소드는 작업이 큐에 의해 실행되었을때 호출될 것입니다. 여러분은 `handle` 메소드에서 필요로 하는 어떤 dependency들을  type-hint 할 수 있으며 [service container](/docs/5.0/container) 는 자동으로 그것들을 추가할 것입니다:
...
만약 여러분의 커멘드가 분리된 핸들러(handler) 클래스를 가지기를 원한다면 `--handler` flag를 `make:command` 커멘드에 추가할 수 있습니다.
...
때때로 여러분들은 큐 작업을 지연시키기를 원합니다. 예를들어 고객이 가입후 15분 후에 이메일을 보내도록 큐 작업을 대기시키길 원할 수 있습니다.  `Queue::later` 메소드를 사용하면 이와 같은 작업을 수행할 수 있습니다:
...
만약 대기된 작업이 구조체 안에서 Eloquent 모델을 받아들인다면 모델에 대한 식별자는 큐에 직렬화(serialized) 됩니다. 
...
작업이 실제로 처리될때 큐 시스템은 전체 모델 인스턴스를 데이터베이스로부터 자동으로 다시  검색할 것입니다.  
...
이러한 방법은 여러분의 어플리케이션에 대해 완전히 알기쉽게 하고 전체 Eloquent 모델 인스턴스를 직렬화(serializing) 하는데서 발생할 수 있는 이슈들을 막습니다.
...
만약 작업을 수동으로 `delete` 혹은 `release` 하기 원한다면,  `Illuminate\Queue\InteractsWithQueue` trait 는 큐 작업의 `release` 와 `delete` 메소드에 대한 접근 방법을 제공합니다 
`release` 메소드는 작업이 다시 할 수 있을때까지 기다리는 시간의 값을 받습니다.
...
만약 예외처리가 작업이 진행하는동안 발생한다면 이 작업은 자동으로 다시 시도될 수 있도록 큐에 다시 보내집니다. 
The job will continue to be released until it has been attempted the maximum number of times allowed by your application. 작업은 여러분의 어플리케이션에서 최대 허용된 횟수까지 반복되어 시도됩니다.  
...
> **참고** 이 메소드를 호출하기 위해 커맨드 / 핸들러는 `Illuminate\Queue\InteractsWithQueue` trait 를 반드시 사용해야만 합니다.
...
큐(Queue)에는 클로져 또한 등록할 수 있습니다. 이것은 빠르고 간단한 작업을 큐로 처리해야할 필요가 있을 경우 매우 에 편리합니다.
...
Iron.io [push queues](#push-queues) 를 사용할때는 추가로 예방 조치 queueing Closures를 취해야 합니다. 큐 메세지를 받는 엔드포인트(end-point)는 요청이 실제로 Iron.io에서 왔는지 검증하는 토큰을 체크해야합니다. 예를들어 push queue end-point 는 `https://yourapp.com/queue/receive?token=SecretToken`와 같이 되어야 합니다.  그 다음 여러분의 어플리케이션에서 큐 요청을 marshalling 하기 전에  시크릿 토큰의 값을 확인할 수 있습니다.
...
listener가 활용해야하는 큐 연결을 지정할 수 도 있습니다:
...
참고로 작업이 시작되면 작업은 수동으로 정지될때까지 동작될 것입니다. [Supervisor](http://supervisord.org/) 와 같은 프로세스 모니터를 사용하여 반드시 큐 리스너가 작업을 중단하지 않도록 해야합니다.
...
큐의 첫번째 작업만을 처리하기 위해서는 `queue:work` 명령을 사용할 수 있습니다.
...
`queue:work`는 또한 어떤 프레임워크의 재부팅 없이 지속적으로 작업을 처리하도록 queue worker 를 강요하는 `--daemon` 옵션이 포함되어있습니다.  `queue:listen` 명령과 비교하여 CPU 사용이 현저하게 줄일 수 있습니다. , but at the added complexity of needing to drain the queues of currently executing jobs during your deployments.

To start a queue worker in daemon mode, use the `--daemon` flag:
queue worker 를 데몬에서 시작하기 위해 `--daemon` flag를 사용할 수 있습니다.
...
여러분이 보신것처럼 `queue:work` 명령어는 `queue:listen`에서 이용가능한 대부분의 같은 옵션들을 제공하고 있습니다.  `php artisan help queue:work` 명령어를 사용하면 사용 가능한 모든 옵션들을 볼 수 있습니다.
...
데몬 queue worker 를 사용하여 어플리케이션을 deploy하기 위한 가장 쉬운 방법은 배포를 시작하는 시점에  `php artisan down` 명령을 사용하여 어플리케이션을 유지보수 모드에 두는것입니다.  어플리케이션이 유지보수 모드에 들어가면 라라벨은 어떤 새 작업들도 받아들이지 않을 것입니다. 다만 남아있던 작업들은 계속해서 진행됩니다.
...
데몬 큐  workers는 각각의 작업이 진행되기 전에 프레임워크를 재시작하지 않습니다. 그렇기에 여러분의 작업이 마치기 전 큰 자원을 확보하기위해 주의해야합니다. 예를들어 GD 라이브러리로 이미지 처리를 하고 있다면 작업이 종료된 후 `imagedestroy`로 메모리를 비워주어야 합니다.

Similarly, your database connection may disconnect when being used by long-running daemon. You may use the `DB::reconnect` method to ensure you have a fresh connection.
비슷하게 데이터베이스 연결은 데몬이 장시간 동작할때 끊어질 수 있습니다. 여러분은 `DB::reconnect` 메소드를 사용하여 깨끗한 상태의 연결을 가지도록 할 수 있습니다.
...
`marshal` 메소드는 올바른 작업 핸들러 클래스가 작동(firing)되는것을 돌볼 것입니다. 푸쉬 큐에서 작업들을 처리하려면 기존에 큐에서 사용한것과 같은 `Queue::push` 메소드를 사용하십시오.
...
모든일들이 항상 계획한것처럼 진행되지 않기 때문에 종종 여러분들의 큐 작업은 실패하기도 합니다. 걱정하지 마세요. 이것은 누구에게나 생길 수 있는 일입니다! 라라벨은 작업이 시도되는 최대 횟수를 지정하는 편리한 방법을 제공합니다. 작업이 제한된 횟수를 초과하는 경우 이 작업들은 `failed_jobs` 테이블에 추가됩니다. 이렇게 실패한 작업들이 담기는 테이블의 이름은 `config/queue.php` 환경설정 파일에서 설정하실 수 있습니다.
...
만약 작업이 실패되었을때 호출되는 이벤트를 등록하고 싶다면  `Queue::failing` 메소드를 사용할 수 있습니다. 이 이벤트는 여러분의 팀에게 이메일 혹은 [HipChat](https://www.hipchat.com)으로 알릴 수 있는 좋은 기회(opportunity)입니다.
