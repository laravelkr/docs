...
라라벨과 다른 퍼스트 파티 패키지들은 [유의적 버젼](https://semver.org/lang/ko/)을 따릅니다. 메이저 프레임워크 릴리즈는 6 개월마다 (2월, 8월) 릴리즈되며, 마이너 릴리스는 매주 여러번 릴리즈 될 수 있습니다. 마이너 릴리즈에는 이전 버전의 호환성을 깨뜨리는 변경 사항이 **없어야** 합니다.
...
라라벨 6.0 (LTS) 은 [Laravel Vapor](https://vapor.laravel.com)과의 호환성, 권한 응답, job 미들웨어, lazy 콜렉션, 서브쿼리 개선, `laravel/ui` 패키지를 통한 프론트엔드 스캐폴딩 및 다양한 버그 수정 및 유용성 개선 등을 통해 라라벨 5.8 의 만들어진 개선을 지속할 것입니다.
...
라라벨 프레임워크 (`laravel/framework`) 패키지는 [유의적 버젼](https://semver.org/lang/ko/) 표준을 따릅니다. 이것은 이 버젼 표준을 따른 다른 퍼스트 파티 패키지들와 함께 프레임워크 일관성을 만듭니다. 라라벨 릴리즈 사이클은 변경되지 않을 것입니다.
...
라라벨 6.0 은 라라벨를 위한 '오토스케일링 서버리스 개발 플랫폼'인 [라라벨 Vapor](https://vapor.laravel.com)과 호환성을 제공합니다. Vapor 는 SQS 큐들, 데이타베이스들, Redis 클러스터들, 네트워크들, CloudFront CDN 등과 이외의 것들을 인터페이스화해서 AWS Lambda 위에서의 라라벨 어플리케이션 관리의 복잡함을 추상화하였습니다.
...
Job 미들웨어는 큐에 들어간 job 이 실행될때, 그 전후를 감싸는 커스텀 로직을 추가 가능하게 합니다. 예를 들자면, 당신은 라라벨 이전 버젼에서 실행속도 제한(rated-limited) 콜백에서 그 job 의 `handle` 메소드의 로직 전후를 감쌌야 했을 겁니다:
...
많은 개발자들은 이미 라라벨의 강력한 [콜렉션 메소드들](https://laravel.com/docs/collections) 잘 사용하고 있습니다. 이 강력한 `Collection` 클래스를 좀더 지원하기 위해 라라벨 6.0 에서는 PHP 의 [generators](https://www.php.net/manual/en/language.generators.overview.php)를 이용한, 적은 메모리 사용을 유지하먼서 아주 큰 데이타셋과 함께 일하기 맞춤인 Lazy 콜렉션을 도입했습니다. 

For example, imagine your application needs to process a multi-gigabyte log file while taking advantage of Laravel's collection methods to parse the logs. Instead of reading the entire file into memory at once, lazy collections may be used to keep only a small part of the file in memory at a given time:

예를 들자면, 당신의 어플리케이션이 로그를 파싱하기위해 라라벨의 콜렉션 메소드들 이용함으로써 얻는 이익이 있는 가운데, 몇 기가바이트 단위의 로그 파일을 처리해야함을 가정해보세요. 파일 전체를 메모리 안으로 한번에 읽어드리는 대신, Lazy 콜렉션은 주어진 시간 안에 메모리로 파일의 일부분 로딩을 유지할 것입니다:
...
혹은 당신이 만개의 엘로퀜트 모델을 순회해야 함을 가정합시다. 기존의 라라벨 콜렉션을 이용했다면 전체 만개의 엘로퀜트 모델은 매번 메모리에 로딩되어져야 할 것입니다:
...
그러나 라라벨 6.0 의 시작과함게, 쿼리빌더의 `cursor` 메소드는 `LazyCollection` 인스턴스로 반환됨으로 변경되었습니다. 데이타베이스에 하나의 쿼리에 실행해 한번에 메모리에 하나의 엘로퀜트 모델의 유지하도록 합니다. 예를 들자면 `filter` 콜백은 각 사용자 개별로 순회 반복되기전까지 실행되지 않아서 메모리 사용량이 크게 줄어듭니다.
...
라라벨 6.0 은 데이타베이스 서브쿼리 지원에 대한 새로운 확장과 향상을 소개했습니다. 예를 들자면, 비행 `destinations`(목적지) 테이블과 목적지로의 `flights`(비행) 테이블이 있다고 상상해봅시다.
...
더해서, 해당 목적지에 도착한 마지막 비행 순서에 기초해서, 모든 도착지를 정열할수 있는 기능을 쿼리빌더의 `orderBy`에 추가하는 새로운 서브쿼리 기능을 사용할수도 있습니다.  
...
라라벨 전 릴리즈에 제공되었던 전통적인 프론트엔드 스캐폴딩이 `laravel\ui` 컴포저 패키지로 추출되었습니다. 이 것은 주 프레임웍으로부터 개발 분리 될 퍼스트 파티 UI 스캐폴딩이 될 것입니다. 이 같은 변화의 결과로 bootstrap, vue 코드가 기본 프레임웍 스캐폴딩에서 제공되지 않고, `make:auth` 명령어도 프레임웍에서 추출되었습니다.
