...
각각의 엘로퀀트 모델은 그 모델에 대한 모든 검색 레코드를 가지고 있는 검색 "인덱스"와 동기화 됩니다. 다시 말해서, 여러분은 MySQL 테이블처럼 각 인덱스를 생각할 수 있습니다. 기본적으로 각 모델은 그 모델의 "테이블" 명과 매칭되는 인덱스명을 유지합니다. 전형적으로 인덱스명은 모델명의 복수형입니다; 하지만 여러분은 모델의 `searchableAs` 메소드를 오버라이드하여 인덱스명을 자유롭게 변경할 수 있습니다.
